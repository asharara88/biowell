...
const MultiGoalSelector: React.FC<MultiGoalSelectorProps> = ({ 
  onGoalsChange, 
  maxGoals = 3,
  ...
  // Initialize priorities for initial goals
  useEffect(() => {
    if (initialGoals.length > 0) {
      const initialPriorities: Record<string, number> = {};
      initialGoals.forEach((goalId, index) => {
        initialPriorities[goalId] = index + 1;
      });
      setGoalPriorities(initialPriorities);
    }
  }, [initialGoals]);

  // Load saved goals on component mount if no initial goals provided
  useEffect(() => {
    if (initialGoals.length === 0) {
      loadSavedGoals();
    }
    ...
    try {
      ...
      if (data?.goals) {
        ...
        if (parsedGoals.priorities && typeof parsedGoals.priorities === 'object') {
          ...
          if (parsedGoals.selectedGoals && Array.isArray(parsedGoals.selectedGoals)) {
            parsedGoals.selectedGoals.forEach((goalId: string, index: number) => {
              newPriorities[goalId] = index + 1;
            });
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      <div className="flex items-center justify-between">
        <h3 className="text-xl font-semibold text-white">Select Your Wellness Goals</h3>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex flex-col">
                      <button 
                        ...
                        disabled={goalPriorities[goalId] === 1 || isLoading || isSaving}
                      ...
                      <button 
                        ...
                        disabled={goalPriorities[goalId] === selectedGoals.length || isLoading || isSaving}
                        className="text-gray-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed p-0.5"
                      >
...
export default MultiGoalSelector;